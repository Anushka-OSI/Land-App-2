...
<section class="news-section" id="news-events" aria-label="Latest news and events">
    <div class="container">
        ...
        <div class="news-header">
            <div class="reveal">
                <span
                    style="display:inline-block;font-family:'Outfit',sans-serif;font-size:.75rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#2d7a4f;background:#f0faf5;border:1px solid #d4f0e0;padding:.3rem .9rem;border-radius:999px;margin-bottom:.9rem;">Stay
                    Informed</span>
            ...
            <div class="reveal" style="display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap;">
                <div class="news-tabs" role="tablist" aria-label="News categories">
                    <button class="tab-btn active" data-tab="all" role="tab" aria-selected="true"
                        aria-controls="tab-all" id="tab-btn-all">All</button>
                    <button class="tab-btn" data-tab="news" role="tab" aria-selected="false" aria-controls="tab-news"
                        id="tab-btn-news">News</button>
                    <button class="tab-btn" data-tab="notice" role="tab" aria-selected="false"
                        aria-controls="tab-notice" id="tab-btn-notice">Notices</button>
                    <button class="tab-btn" data-tab="event" role="tab" aria-selected="false" aria-controls="tab-event"
                        id="tab-btn-event">Events</button>
                </div>
            </div>
        </div>
        ...
        <div class="news-panel active" id="tab-all" role="tabpanel" aria-labelledby="tab-btn-all">
            <div class="news-grid">
                @foreach($news as $item)
                    <article class="news-card {{ strtolower($item['badge']) }} reveal" tabindex="0"
                        ...
                        <div class="news-card-body">
                            ...
                            <a href="#" class="news-read-more" id="news-item-{{ $item['id'] }}"
                        ...
                        <div class="news-card-body">
                            <div class="news-card-meta">
                                ...
                                <time class="news-date"
                                    datetime="{{ $item['date'] }}">{{ \Carbon\Carbon::parse($item['date'])->format('M d, Y') }}</time>
                            </div>
                            <h3 class="news-card-title">{{ $item['title'] }}</h3>
                            <p class="news-card-excerpt">{{ $item['excerpt'] }}</p>
                            <a href="#" class="news-read-more">Read More <svg xmlns="http://www.w3.org/2000/svg" fill="none"
